angular.module('ppApp.service.firebase.changeEmail', ['ppApp.service.firebase'])
  .factory('changeEmail', ['firebaseSync', '$q', function(firebaseSync, $q) {
    return function(password, oldEmail, newEmail, loginService) {
      ...
      function authOldAccount() {
        return loginService.login(ctx.old.email, password).then(function(user) {
      ...
      function loadOldProfile() {
        var def = $q.defer();
        ctx.old.ref = firebaseSync.ref('users', ctx.old.uid);
      ...
      function createNewAccount() {
        return loginService.createAccount(ctx.curr.email, password, ctx.old.name).then(function(user) {
      ...
      function copyProfile() {
        var d = $q.defer();
        ctx.curr.ref = firebaseSync.ref('users', ctx.curr.uid);
      ...
      function removeOldLogin() {
        var def = $q.defer();
        loginService.removeUser(ctx.old.email, password).then(function() {
          def.resolve();
        }, function(err) {
          def.reject(err);
        });
