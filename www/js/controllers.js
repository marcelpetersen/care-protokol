var ppAppCtrl = angular.module('ppApp.controllers', []);


ppAppCtrl.controller('AppCtrl', function($scope, loginService) {

    $scope.user = loginService.user;
    $scope.userRoles = loginService.userRoles;
    $scope.accessLevels = loginService.accessLevels;

    // should be in CareController
    $scope.getDaysliceText = function(dayslice) {
      switch (dayslice) {
        case 'VM' :  return "Hilfe am Vormittag";
        case 'MI' :  return "Hilfe gegen Mittag";
        case 'AB' :  return "Hilfe am Abend";
        case 'NA' :  return "Hilfe in der Nacht";
        default: return "Bitte Tageszeit auswählen";
      }
    }

    $scope.getActivityEntryText = function(entry) {
      var text ='';
      switch (entry.type) {
        case 'UN' : text = text + "Unterstützung"; break;
        case 'TU' : text = text + "Teilweise Übernahme"; break;
        case 'VU' : text = text + "Vollständige Übernahme"; break;
        case 'BE' : text = text + "Begleitung und Anleitung"; break;
      };
/*
      switch (entry.dayslice) {
        case 'VM' :  text = text + " am Vormittag"; break;
        case 'MI' :  text = text + " gegen Mittag"; break;
        case 'AB' :  text = text + " am Abend"; break;
        case 'NA' :  text = text + " in der Nacht"; break;
      };
*/
      return text;
    }
  ...
  .controller('HomeCtrl', function($scope, $rootScope, Family, careCount, drugCount, drugList, careState, competencyState, medicationState, tipState, loginService, rewardService) {

    $scope.user = loginService.user;
    $scope.userRoles = loginService.userRoles;
    $scope.accessLevels = loginService.accessLevels;
    $scope.drugList = drugList;
    $scope.careCount = careCount;
    $scope.drugCount = drugCount;
    $scope.careState = careState;
    $scope.competencyState = competencyState;
    $scope.medicationState = medicationState;
    $scope.tipState = tipState;

    //$scope.badges = [ "shade", "tree", "way"];
  ...
  .controller('LanguageCtrl', function($scope, $ionicSideMenuDelegate, $translate, $log) {
    //$log.debug("entered LanguageCtrl");
    $scope.changeLanguage = function (key) {
      $translate.use(key).then(function (key) {
        //console.log("Language changed to " + key);
      }, function (key) {
        //console.log("Language changing to " + key +" failed");
      });
    };
    $scope.isCurrentLanguage = function (key) {
...
ppAppCtrl.controller('ContactsCtrl', function($scope, $ionicScrollDelegate, $http, $ionicLoading, careActivitiesDataService, loginService, usersService) {
  ...
  var family;
  var _allFamilyActivities;
  loginService.getUser().then(function(currentUser){
    if(currentUser !== null) {
      usersService.getFamily(currentUser).then(function(famObj){
        family = famObj.$value;
        _allFamilyActivities = careActivitiesDataService.getAllFamilyActivities(family, null, null);
        $scope.allFamilyActivities = _allFamilyActivities;
      });
    }
  });
  ...
  $scope.getItemAsArray = function(item) {
    console.log('-- item -- ');
    console.log(item);
    var arr = [];
